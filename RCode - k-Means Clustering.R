...
## K-means clustering
?kmeans
fit <- kmeans(standardized_data, centers=3, iter.max=10, nstart=10)
...
for (i in 1:5) Cluster_Variability[i] <- kmeans(standardized_data,centers=i, nstart=4)$tot.withinss
...
fit <- kmeans(standardized_data, centers=4, iter.max=20, nstart=20)
...
NormDatasWithClusterMembership <- cbind(standardized_data, fit$cluster) # append cluster membership
...
## some trivial analysis
fit <- kmeans(standardized_data, centers=2, iter.max=10, nstart=4)
